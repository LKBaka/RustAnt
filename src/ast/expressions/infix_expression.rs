...
impl Node for InfixExpression {
    ...
    fn eval(&mut self, env: &mut Environment) -> Option<Box<dyn IAntObject>> {
        let left_obj = self.left.eval(env);
        if is_error(left_obj.clone()?) {return Some(left_obj?)}

        let right_obj = self.right.eval(env);
        if is_error(right_obj.clone()?) {return Some(right_obj?)}

        call_function_with_name(OPERATOR_TO_FUNCTION_NAME_MAP[&self.operator.clone()].to_string(), vec![left_obj.clone()?, right_obj?], &mut left_obj?.get_env())
    }
}
