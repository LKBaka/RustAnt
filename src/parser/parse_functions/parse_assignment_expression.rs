...
pub fn parse_assignment_expression(parser: &mut Parser, left: Box<dyn Expression>) -> Option<Box<dyn Expression>> {
    ...
    if let Some(value) = value {
        return Some(Box::new(create_assignment_expression(token, left_expression, value)))
    }
