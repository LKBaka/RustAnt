...
pub fn parse_infix_expression(parser: &mut Parser, left: Box<dyn Expression>) -> Option<Box<dyn Expression>> {
    ...
    let right_expression = parser.parse_expression(
        if operator == "+" {
            precedence - 1
        } else {
            precedence
    ...
    match right_expression {
        ...
        Some(right_expression) => {
            Some(
                Box::new(create_infix_expression(token, left_expression, right_expression, operator))
            )
        }
    }
}